# auto-generated - v_definition_python_dtos_write_list - START at 2024-03-29 22:25:24.841778+01
...
@dataclass(frozen=False)
class system_object_write_dtos(base_write_dtos):
    ...
    def map_by_uid(self) -> dict[str, system_object_write_dto]:
        ...
        return res
    def get_active(self):
        return system_object_write_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self):
        return system_object_write_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> system_object_write_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> system_object_write_dtos:
        if len(self.dtos) > n:
            return system_object_write_dtos(self.dtos[:n])
        else:
            return system_object_write_dtos(self.dtos)
    def get_last(self) -> system_object_write_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, system_object_write_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[system_object_write_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[system_object_write_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[system_object_write_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[system_object_write_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[system_object_write_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[system_object_write_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, system_object_write_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, system_object_write_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, system_object_write_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[system_object_write_dto], bool]) -> system_object_write_dtos:
        return system_object_write_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[system_object_write_dto], str]) -> dict[str, system_object_write_dtos]:
        grp_data: dict[str, system_object_write_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = system_object_write_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[system_object_write_dto], str], agg_method: Callable[[system_object_write_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, system_object_write_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[system_object_write_dto], bool]) -> system_object_write_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[system_object_write_dto], any], compare_method: Callable[[any, any], bool]) -> system_object_write_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class system_object_type_write_dtos(base_write_dtos):
    ...
    def map_by_uid(self) -> dict[str, system_object_type_write_dto]:
        ...
        return res
    def get_active(self):
        return system_object_type_write_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self):
        return system_object_type_write_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> system_object_type_write_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> system_object_type_write_dtos:
        if len(self.dtos) > n:
            return system_object_type_write_dtos(self.dtos[:n])
        else:
            return system_object_type_write_dtos(self.dtos)
    def get_last(self) -> system_object_type_write_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, system_object_type_write_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[system_object_type_write_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[system_object_type_write_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[system_object_type_write_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[system_object_type_write_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[system_object_type_write_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[system_object_type_write_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, system_object_type_write_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, system_object_type_write_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, system_object_type_write_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[system_object_type_write_dto], bool]) -> system_object_type_write_dtos:
        return system_object_type_write_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[system_object_type_write_dto], str]) -> dict[str, system_object_type_write_dtos]:
        grp_data: dict[str, system_object_type_write_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = system_object_type_write_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[system_object_type_write_dto], str], agg_method: Callable[[system_object_type_write_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, system_object_type_write_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[system_object_type_write_dto], bool]) -> system_object_type_write_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[system_object_type_write_dto], any], compare_method: Callable[[any, any], bool]) -> system_object_type_write_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
