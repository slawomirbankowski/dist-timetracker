# auto-generated - v_definition_python_dtos_rich_list - START at 2024-08-04 09:35:47.433214+00
...
@dataclass(frozen=False)
class account_skill_assignment_rich_dtos(base_dtos):
    dtos: list[account_skill_assignment_rich_dto]
    def __init__(self, dtos: list[account_skill_assignment_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[account_skill_assignment_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: account_skill_assignment_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[account_skill_assignment_rich_dto], dtos2: list[account_skill_assignment_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> account_skill_assignment_rich_dto | None:
        found_dtos = list(filter(lambda x: x.account_skill_assignment_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, account_skill_assignment_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.account_skill_assignment_uid] = dto
        return res
    def get_active(self) -> account_skill_assignment_rich_dtos:
        return account_skill_assignment_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> account_skill_assignment_rich_dtos:
        return account_skill_assignment_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> account_skill_assignment_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> account_skill_assignment_rich_dtos:
        if len(self.dtos) > n:
            return account_skill_assignment_rich_dtos(self.dtos[:n])
        else:
            return account_skill_assignment_rich_dtos(self.dtos)
    def get_last(self) -> account_skill_assignment_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, account_skill_assignment_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[account_skill_assignment_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[account_skill_assignment_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[account_skill_assignment_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[account_skill_assignment_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[account_skill_assignment_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[account_skill_assignment_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, account_skill_assignment_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, account_skill_assignment_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, account_skill_assignment_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[account_skill_assignment_rich_dto], bool]) -> account_skill_assignment_rich_dtos:
        return account_skill_assignment_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[account_skill_assignment_rich_dto], str]) -> dict[str, account_skill_assignment_rich_dtos]:
        grp_data: dict[str, account_skill_assignment_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = account_skill_assignment_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[account_skill_assignment_rich_dto], str], agg_method: Callable[[account_skill_assignment_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, account_skill_assignment_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[account_skill_assignment_rich_dto], bool]) -> account_skill_assignment_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[account_skill_assignment_rich_dto], any], compare_method: Callable[[any, any], bool]) -> account_skill_assignment_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class account_skill_group_rich_dtos(base_dtos):
    dtos: list[account_skill_group_rich_dto]
    def __init__(self, dtos: list[account_skill_group_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[account_skill_group_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: account_skill_group_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[account_skill_group_rich_dto], dtos2: list[account_skill_group_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> account_skill_group_rich_dto | None:
        found_dtos = list(filter(lambda x: x.account_skill_group_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, account_skill_group_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.account_skill_group_uid] = dto
        return res
    def get_active(self) -> account_skill_group_rich_dtos:
        return account_skill_group_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> account_skill_group_rich_dtos:
        return account_skill_group_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> account_skill_group_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> account_skill_group_rich_dtos:
        if len(self.dtos) > n:
            return account_skill_group_rich_dtos(self.dtos[:n])
        else:
            return account_skill_group_rich_dtos(self.dtos)
    def get_last(self) -> account_skill_group_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, account_skill_group_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[account_skill_group_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[account_skill_group_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[account_skill_group_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[account_skill_group_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[account_skill_group_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[account_skill_group_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, account_skill_group_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, account_skill_group_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, account_skill_group_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[account_skill_group_rich_dto], bool]) -> account_skill_group_rich_dtos:
        return account_skill_group_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[account_skill_group_rich_dto], str]) -> dict[str, account_skill_group_rich_dtos]:
        grp_data: dict[str, account_skill_group_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = account_skill_group_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[account_skill_group_rich_dto], str], agg_method: Callable[[account_skill_group_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, account_skill_group_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[account_skill_group_rich_dto], bool]) -> account_skill_group_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[account_skill_group_rich_dto], any], compare_method: Callable[[any, any], bool]) -> account_skill_group_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class auth_permission_type_rich_dtos(base_dtos):
    dtos: list[auth_permission_type_rich_dto]
    def __init__(self, dtos: list[auth_permission_type_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[auth_permission_type_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: auth_permission_type_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[auth_permission_type_rich_dto], dtos2: list[auth_permission_type_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> auth_permission_type_rich_dto | None:
        found_dtos = list(filter(lambda x: x.auth_permission_type_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, auth_permission_type_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.auth_permission_type_uid] = dto
        return res
    def get_active(self) -> auth_permission_type_rich_dtos:
        return auth_permission_type_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> auth_permission_type_rich_dtos:
        return auth_permission_type_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> auth_permission_type_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> auth_permission_type_rich_dtos:
        if len(self.dtos) > n:
            return auth_permission_type_rich_dtos(self.dtos[:n])
        else:
            return auth_permission_type_rich_dtos(self.dtos)
    def get_last(self) -> auth_permission_type_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, auth_permission_type_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[auth_permission_type_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[auth_permission_type_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[auth_permission_type_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[auth_permission_type_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[auth_permission_type_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[auth_permission_type_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, auth_permission_type_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, auth_permission_type_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, auth_permission_type_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[auth_permission_type_rich_dto], bool]) -> auth_permission_type_rich_dtos:
        return auth_permission_type_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[auth_permission_type_rich_dto], str]) -> dict[str, auth_permission_type_rich_dtos]:
        grp_data: dict[str, auth_permission_type_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = auth_permission_type_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[auth_permission_type_rich_dto], str], agg_method: Callable[[auth_permission_type_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, auth_permission_type_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[auth_permission_type_rich_dto], bool]) -> auth_permission_type_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[auth_permission_type_rich_dto], any], compare_method: Callable[[any, any], bool]) -> auth_permission_type_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class auth_pin_rich_dtos(base_dtos):
    dtos: list[auth_pin_rich_dto]
    def __init__(self, dtos: list[auth_pin_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[auth_pin_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: auth_pin_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[auth_pin_rich_dto], dtos2: list[auth_pin_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> auth_pin_rich_dto | None:
        found_dtos = list(filter(lambda x: x.auth_pin_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, auth_pin_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.auth_pin_uid] = dto
        return res
    def get_active(self) -> auth_pin_rich_dtos:
        return auth_pin_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> auth_pin_rich_dtos:
        return auth_pin_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> auth_pin_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> auth_pin_rich_dtos:
        if len(self.dtos) > n:
            return auth_pin_rich_dtos(self.dtos[:n])
        else:
            return auth_pin_rich_dtos(self.dtos)
    def get_last(self) -> auth_pin_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, auth_pin_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[auth_pin_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[auth_pin_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[auth_pin_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[auth_pin_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[auth_pin_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[auth_pin_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, auth_pin_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, auth_pin_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, auth_pin_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[auth_pin_rich_dto], bool]) -> auth_pin_rich_dtos:
        return auth_pin_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[auth_pin_rich_dto], str]) -> dict[str, auth_pin_rich_dtos]:
        grp_data: dict[str, auth_pin_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = auth_pin_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[auth_pin_rich_dto], str], agg_method: Callable[[auth_pin_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, auth_pin_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[auth_pin_rich_dto], bool]) -> auth_pin_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[auth_pin_rich_dto], any], compare_method: Callable[[any, any], bool]) -> auth_pin_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class auth_token_type_rich_dtos(base_dtos):
    dtos: list[auth_token_type_rich_dto]
    def __init__(self, dtos: list[auth_token_type_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[auth_token_type_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: auth_token_type_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[auth_token_type_rich_dto], dtos2: list[auth_token_type_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> auth_token_type_rich_dto | None:
        found_dtos = list(filter(lambda x: x.auth_token_type_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, auth_token_type_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.auth_token_type_uid] = dto
        return res
    def get_active(self) -> auth_token_type_rich_dtos:
        return auth_token_type_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> auth_token_type_rich_dtos:
        return auth_token_type_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> auth_token_type_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> auth_token_type_rich_dtos:
        if len(self.dtos) > n:
            return auth_token_type_rich_dtos(self.dtos[:n])
        else:
            return auth_token_type_rich_dtos(self.dtos)
    def get_last(self) -> auth_token_type_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, auth_token_type_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[auth_token_type_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[auth_token_type_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[auth_token_type_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[auth_token_type_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[auth_token_type_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[auth_token_type_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, auth_token_type_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, auth_token_type_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, auth_token_type_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[auth_token_type_rich_dto], bool]) -> auth_token_type_rich_dtos:
        return auth_token_type_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[auth_token_type_rich_dto], str]) -> dict[str, auth_token_type_rich_dtos]:
        grp_data: dict[str, auth_token_type_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = auth_token_type_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[auth_token_type_rich_dto], str], agg_method: Callable[[auth_token_type_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, auth_token_type_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[auth_token_type_rich_dto], bool]) -> auth_token_type_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[auth_token_type_rich_dto], any], compare_method: Callable[[any, any], bool]) -> auth_token_type_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class client_contract_rich_dtos(base_dtos):
    dtos: list[client_contract_rich_dto]
    def __init__(self, dtos: list[client_contract_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[client_contract_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: client_contract_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[client_contract_rich_dto], dtos2: list[client_contract_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> client_contract_rich_dto | None:
        found_dtos = list(filter(lambda x: x.client_contract_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, client_contract_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.client_contract_uid] = dto
        return res
    def get_active(self) -> client_contract_rich_dtos:
        return client_contract_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> client_contract_rich_dtos:
        return client_contract_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> client_contract_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> client_contract_rich_dtos:
        if len(self.dtos) > n:
            return client_contract_rich_dtos(self.dtos[:n])
        else:
            return client_contract_rich_dtos(self.dtos)
    def get_last(self) -> client_contract_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, client_contract_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[client_contract_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[client_contract_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[client_contract_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[client_contract_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[client_contract_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[client_contract_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, client_contract_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, client_contract_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, client_contract_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[client_contract_rich_dto], bool]) -> client_contract_rich_dtos:
        return client_contract_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[client_contract_rich_dto], str]) -> dict[str, client_contract_rich_dtos]:
        grp_data: dict[str, client_contract_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = client_contract_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[client_contract_rich_dto], str], agg_method: Callable[[client_contract_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, client_contract_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[client_contract_rich_dto], bool]) -> client_contract_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[client_contract_rich_dto], any], compare_method: Callable[[any, any], bool]) -> client_contract_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class competency_entry_rich_dtos(base_dtos):
    dtos: list[competency_entry_rich_dto]
    def __init__(self, dtos: list[competency_entry_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[competency_entry_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: competency_entry_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[competency_entry_rich_dto], dtos2: list[competency_entry_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> competency_entry_rich_dto | None:
        found_dtos = list(filter(lambda x: x.competency_entry_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, competency_entry_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.competency_entry_uid] = dto
        return res
    def get_active(self) -> competency_entry_rich_dtos:
        return competency_entry_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> competency_entry_rich_dtos:
        return competency_entry_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> competency_entry_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> competency_entry_rich_dtos:
        if len(self.dtos) > n:
            return competency_entry_rich_dtos(self.dtos[:n])
        else:
            return competency_entry_rich_dtos(self.dtos)
    def get_last(self) -> competency_entry_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, competency_entry_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[competency_entry_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[competency_entry_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[competency_entry_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[competency_entry_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[competency_entry_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[competency_entry_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, competency_entry_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, competency_entry_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, competency_entry_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[competency_entry_rich_dto], bool]) -> competency_entry_rich_dtos:
        return competency_entry_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[competency_entry_rich_dto], str]) -> dict[str, competency_entry_rich_dtos]:
        grp_data: dict[str, competency_entry_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = competency_entry_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[competency_entry_rich_dto], str], agg_method: Callable[[competency_entry_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, competency_entry_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[competency_entry_rich_dto], bool]) -> competency_entry_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[competency_entry_rich_dto], any], compare_method: Callable[[any, any], bool]) -> competency_entry_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class competency_entry_account_rich_dtos(base_dtos):
    dtos: list[competency_entry_account_rich_dto]
    def __init__(self, dtos: list[competency_entry_account_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[competency_entry_account_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: competency_entry_account_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[competency_entry_account_rich_dto], dtos2: list[competency_entry_account_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> competency_entry_account_rich_dto | None:
        found_dtos = list(filter(lambda x: x.competency_entry_account_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, competency_entry_account_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.competency_entry_account_uid] = dto
        return res
    def get_active(self) -> competency_entry_account_rich_dtos:
        return competency_entry_account_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> competency_entry_account_rich_dtos:
        return competency_entry_account_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> competency_entry_account_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> competency_entry_account_rich_dtos:
        if len(self.dtos) > n:
            return competency_entry_account_rich_dtos(self.dtos[:n])
        else:
            return competency_entry_account_rich_dtos(self.dtos)
    def get_last(self) -> competency_entry_account_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, competency_entry_account_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[competency_entry_account_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[competency_entry_account_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[competency_entry_account_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[competency_entry_account_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[competency_entry_account_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[competency_entry_account_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, competency_entry_account_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, competency_entry_account_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, competency_entry_account_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[competency_entry_account_rich_dto], bool]) -> competency_entry_account_rich_dtos:
        return competency_entry_account_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[competency_entry_account_rich_dto], str]) -> dict[str, competency_entry_account_rich_dtos]:
        grp_data: dict[str, competency_entry_account_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = competency_entry_account_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[competency_entry_account_rich_dto], str], agg_method: Callable[[competency_entry_account_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, competency_entry_account_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[competency_entry_account_rich_dto], bool]) -> competency_entry_account_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[competency_entry_account_rich_dto], any], compare_method: Callable[[any, any], bool]) -> competency_entry_account_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class competency_group_rich_dtos(base_dtos):
    dtos: list[competency_group_rich_dto]
    def __init__(self, dtos: list[competency_group_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[competency_group_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: competency_group_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[competency_group_rich_dto], dtos2: list[competency_group_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> competency_group_rich_dto | None:
        found_dtos = list(filter(lambda x: x.competency_group_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, competency_group_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.competency_group_uid] = dto
        return res
    def get_active(self) -> competency_group_rich_dtos:
        return competency_group_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> competency_group_rich_dtos:
        return competency_group_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> competency_group_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> competency_group_rich_dtos:
        if len(self.dtos) > n:
            return competency_group_rich_dtos(self.dtos[:n])
        else:
            return competency_group_rich_dtos(self.dtos)
    def get_last(self) -> competency_group_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, competency_group_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[competency_group_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[competency_group_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[competency_group_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[competency_group_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[competency_group_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[competency_group_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, competency_group_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, competency_group_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, competency_group_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[competency_group_rich_dto], bool]) -> competency_group_rich_dtos:
        return competency_group_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[competency_group_rich_dto], str]) -> dict[str, competency_group_rich_dtos]:
        grp_data: dict[str, competency_group_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = competency_group_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[competency_group_rich_dto], str], agg_method: Callable[[competency_group_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, competency_group_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[competency_group_rich_dto], bool]) -> competency_group_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[competency_group_rich_dto], any], compare_method: Callable[[any, any], bool]) -> competency_group_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class competency_group_account_rich_dtos(base_dtos):
    dtos: list[competency_group_account_rich_dto]
    def __init__(self, dtos: list[competency_group_account_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[competency_group_account_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: competency_group_account_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[competency_group_account_rich_dto], dtos2: list[competency_group_account_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> competency_group_account_rich_dto | None:
        found_dtos = list(filter(lambda x: x.competency_group_account_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, competency_group_account_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.competency_group_account_uid] = dto
        return res
    def get_active(self) -> competency_group_account_rich_dtos:
        return competency_group_account_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> competency_group_account_rich_dtos:
        return competency_group_account_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> competency_group_account_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> competency_group_account_rich_dtos:
        if len(self.dtos) > n:
            return competency_group_account_rich_dtos(self.dtos[:n])
        else:
            return competency_group_account_rich_dtos(self.dtos)
    def get_last(self) -> competency_group_account_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, competency_group_account_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[competency_group_account_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[competency_group_account_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[competency_group_account_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[competency_group_account_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[competency_group_account_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[competency_group_account_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, competency_group_account_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, competency_group_account_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, competency_group_account_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[competency_group_account_rich_dto], bool]) -> competency_group_account_rich_dtos:
        return competency_group_account_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[competency_group_account_rich_dto], str]) -> dict[str, competency_group_account_rich_dtos]:
        grp_data: dict[str, competency_group_account_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = competency_group_account_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[competency_group_account_rich_dto], str], agg_method: Callable[[competency_group_account_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, competency_group_account_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[competency_group_account_rich_dto], bool]) -> competency_group_account_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[competency_group_account_rich_dto], any], compare_method: Callable[[any, any], bool]) -> competency_group_account_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class competency_group_type_rich_dtos(base_dtos):
    dtos: list[competency_group_type_rich_dto]
    def __init__(self, dtos: list[competency_group_type_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[competency_group_type_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: competency_group_type_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[competency_group_type_rich_dto], dtos2: list[competency_group_type_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> competency_group_type_rich_dto | None:
        found_dtos = list(filter(lambda x: x.competency_group_type_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, competency_group_type_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.competency_group_type_uid] = dto
        return res
    def get_active(self) -> competency_group_type_rich_dtos:
        return competency_group_type_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> competency_group_type_rich_dtos:
        return competency_group_type_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> competency_group_type_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> competency_group_type_rich_dtos:
        if len(self.dtos) > n:
            return competency_group_type_rich_dtos(self.dtos[:n])
        else:
            return competency_group_type_rich_dtos(self.dtos)
    def get_last(self) -> competency_group_type_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, competency_group_type_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[competency_group_type_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[competency_group_type_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[competency_group_type_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[competency_group_type_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[competency_group_type_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[competency_group_type_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, competency_group_type_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, competency_group_type_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, competency_group_type_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[competency_group_type_rich_dto], bool]) -> competency_group_type_rich_dtos:
        return competency_group_type_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[competency_group_type_rich_dto], str]) -> dict[str, competency_group_type_rich_dtos]:
        grp_data: dict[str, competency_group_type_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = competency_group_type_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[competency_group_type_rich_dto], str], agg_method: Callable[[competency_group_type_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, competency_group_type_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[competency_group_type_rich_dto], bool]) -> competency_group_type_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[competency_group_type_rich_dto], any], compare_method: Callable[[any, any], bool]) -> competency_group_type_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class competency_item_rich_dtos(base_dtos):
    dtos: list[competency_item_rich_dto]
    def __init__(self, dtos: list[competency_item_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[competency_item_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: competency_item_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[competency_item_rich_dto], dtos2: list[competency_item_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> competency_item_rich_dto | None:
        found_dtos = list(filter(lambda x: x.competency_item_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, competency_item_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.competency_item_uid] = dto
        return res
    def get_active(self) -> competency_item_rich_dtos:
        return competency_item_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> competency_item_rich_dtos:
        return competency_item_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> competency_item_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> competency_item_rich_dtos:
        if len(self.dtos) > n:
            return competency_item_rich_dtos(self.dtos[:n])
        else:
            return competency_item_rich_dtos(self.dtos)
    def get_last(self) -> competency_item_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, competency_item_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[competency_item_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[competency_item_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[competency_item_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[competency_item_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[competency_item_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[competency_item_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, competency_item_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, competency_item_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, competency_item_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[competency_item_rich_dto], bool]) -> competency_item_rich_dtos:
        return competency_item_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[competency_item_rich_dto], str]) -> dict[str, competency_item_rich_dtos]:
        grp_data: dict[str, competency_item_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = competency_item_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[competency_item_rich_dto], str], agg_method: Callable[[competency_item_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, competency_item_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[competency_item_rich_dto], bool]) -> competency_item_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[competency_item_rich_dto], any], compare_method: Callable[[any, any], bool]) -> competency_item_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class competency_item_account_rich_dtos(base_dtos):
    dtos: list[competency_item_account_rich_dto]
    def __init__(self, dtos: list[competency_item_account_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[competency_item_account_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: competency_item_account_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[competency_item_account_rich_dto], dtos2: list[competency_item_account_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> competency_item_account_rich_dto | None:
        found_dtos = list(filter(lambda x: x.competency_item_account_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, competency_item_account_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.competency_item_account_uid] = dto
        return res
    def get_active(self) -> competency_item_account_rich_dtos:
        return competency_item_account_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> competency_item_account_rich_dtos:
        return competency_item_account_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> competency_item_account_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> competency_item_account_rich_dtos:
        if len(self.dtos) > n:
            return competency_item_account_rich_dtos(self.dtos[:n])
        else:
            return competency_item_account_rich_dtos(self.dtos)
    def get_last(self) -> competency_item_account_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, competency_item_account_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[competency_item_account_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[competency_item_account_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[competency_item_account_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[competency_item_account_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[competency_item_account_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[competency_item_account_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, competency_item_account_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, competency_item_account_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, competency_item_account_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[competency_item_account_rich_dto], bool]) -> competency_item_account_rich_dtos:
        return competency_item_account_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[competency_item_account_rich_dto], str]) -> dict[str, competency_item_account_rich_dtos]:
        grp_data: dict[str, competency_item_account_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = competency_item_account_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[competency_item_account_rich_dto], str], agg_method: Callable[[competency_item_account_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, competency_item_account_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[competency_item_account_rich_dto], bool]) -> competency_item_account_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[competency_item_account_rich_dto], any], compare_method: Callable[[any, any], bool]) -> competency_item_account_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class competency_item_type_rich_dtos(base_dtos):
    dtos: list[competency_item_type_rich_dto]
    def __init__(self, dtos: list[competency_item_type_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[competency_item_type_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: competency_item_type_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[competency_item_type_rich_dto], dtos2: list[competency_item_type_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> competency_item_type_rich_dto | None:
        found_dtos = list(filter(lambda x: x.competency_item_type_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, competency_item_type_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.competency_item_type_uid] = dto
        return res
    def get_active(self) -> competency_item_type_rich_dtos:
        return competency_item_type_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> competency_item_type_rich_dtos:
        return competency_item_type_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> competency_item_type_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> competency_item_type_rich_dtos:
        if len(self.dtos) > n:
            return competency_item_type_rich_dtos(self.dtos[:n])
        else:
            return competency_item_type_rich_dtos(self.dtos)
    def get_last(self) -> competency_item_type_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, competency_item_type_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[competency_item_type_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[competency_item_type_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[competency_item_type_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[competency_item_type_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[competency_item_type_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[competency_item_type_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, competency_item_type_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, competency_item_type_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, competency_item_type_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[competency_item_type_rich_dto], bool]) -> competency_item_type_rich_dtos:
        return competency_item_type_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[competency_item_type_rich_dto], str]) -> dict[str, competency_item_type_rich_dtos]:
        grp_data: dict[str, competency_item_type_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = competency_item_type_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[competency_item_type_rich_dto], str], agg_method: Callable[[competency_item_type_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, competency_item_type_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[competency_item_type_rich_dto], bool]) -> competency_item_type_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[competency_item_type_rich_dto], any], compare_method: Callable[[any, any], bool]) -> competency_item_type_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class competency_process_rich_dtos(base_dtos):
    dtos: list[competency_process_rich_dto]
    def __init__(self, dtos: list[competency_process_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[competency_process_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: competency_process_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[competency_process_rich_dto], dtos2: list[competency_process_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> competency_process_rich_dto | None:
        found_dtos = list(filter(lambda x: x.competency_process_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, competency_process_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.competency_process_uid] = dto
        return res
    def get_active(self) -> competency_process_rich_dtos:
        return competency_process_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> competency_process_rich_dtos:
        return competency_process_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> competency_process_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> competency_process_rich_dtos:
        if len(self.dtos) > n:
            return competency_process_rich_dtos(self.dtos[:n])
        else:
            return competency_process_rich_dtos(self.dtos)
    def get_last(self) -> competency_process_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, competency_process_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[competency_process_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[competency_process_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[competency_process_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[competency_process_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[competency_process_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[competency_process_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, competency_process_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, competency_process_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, competency_process_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[competency_process_rich_dto], bool]) -> competency_process_rich_dtos:
        return competency_process_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[competency_process_rich_dto], str]) -> dict[str, competency_process_rich_dtos]:
        grp_data: dict[str, competency_process_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = competency_process_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[competency_process_rich_dto], str], agg_method: Callable[[competency_process_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, competency_process_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[competency_process_rich_dto], bool]) -> competency_process_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[competency_process_rich_dto], any], compare_method: Callable[[any, any], bool]) -> competency_process_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class competency_process_account_rich_dtos(base_dtos):
    dtos: list[competency_process_account_rich_dto]
    def __init__(self, dtos: list[competency_process_account_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[competency_process_account_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: competency_process_account_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[competency_process_account_rich_dto], dtos2: list[competency_process_account_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> competency_process_account_rich_dto | None:
        found_dtos = list(filter(lambda x: x.competency_process_account_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, competency_process_account_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.competency_process_account_uid] = dto
        return res
    def get_active(self) -> competency_process_account_rich_dtos:
        return competency_process_account_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> competency_process_account_rich_dtos:
        return competency_process_account_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> competency_process_account_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> competency_process_account_rich_dtos:
        if len(self.dtos) > n:
            return competency_process_account_rich_dtos(self.dtos[:n])
        else:
            return competency_process_account_rich_dtos(self.dtos)
    def get_last(self) -> competency_process_account_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, competency_process_account_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[competency_process_account_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[competency_process_account_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[competency_process_account_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[competency_process_account_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[competency_process_account_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[competency_process_account_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, competency_process_account_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, competency_process_account_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, competency_process_account_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[competency_process_account_rich_dto], bool]) -> competency_process_account_rich_dtos:
        return competency_process_account_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[competency_process_account_rich_dto], str]) -> dict[str, competency_process_account_rich_dtos]:
        grp_data: dict[str, competency_process_account_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = competency_process_account_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[competency_process_account_rich_dto], str], agg_method: Callable[[competency_process_account_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, competency_process_account_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[competency_process_account_rich_dto], bool]) -> competency_process_account_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[competency_process_account_rich_dto], any], compare_method: Callable[[any, any], bool]) -> competency_process_account_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class competency_process_type_rich_dtos(base_dtos):
    dtos: list[competency_process_type_rich_dto]
    def __init__(self, dtos: list[competency_process_type_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[competency_process_type_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: competency_process_type_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[competency_process_type_rich_dto], dtos2: list[competency_process_type_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> competency_process_type_rich_dto | None:
        found_dtos = list(filter(lambda x: x.competency_process_type_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, competency_process_type_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.competency_process_type_uid] = dto
        return res
    def get_active(self) -> competency_process_type_rich_dtos:
        return competency_process_type_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> competency_process_type_rich_dtos:
        return competency_process_type_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> competency_process_type_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> competency_process_type_rich_dtos:
        if len(self.dtos) > n:
            return competency_process_type_rich_dtos(self.dtos[:n])
        else:
            return competency_process_type_rich_dtos(self.dtos)
    def get_last(self) -> competency_process_type_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, competency_process_type_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[competency_process_type_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[competency_process_type_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[competency_process_type_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[competency_process_type_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[competency_process_type_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[competency_process_type_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, competency_process_type_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, competency_process_type_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, competency_process_type_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[competency_process_type_rich_dto], bool]) -> competency_process_type_rich_dtos:
        return competency_process_type_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[competency_process_type_rich_dto], str]) -> dict[str, competency_process_type_rich_dtos]:
        grp_data: dict[str, competency_process_type_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = competency_process_type_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[competency_process_type_rich_dto], str], agg_method: Callable[[competency_process_type_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, competency_process_type_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[competency_process_type_rich_dto], bool]) -> competency_process_type_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[competency_process_type_rich_dto], any], compare_method: Callable[[any, any], bool]) -> competency_process_type_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class competency_ranking_rich_dtos(base_dtos):
    dtos: list[competency_ranking_rich_dto]
    def __init__(self, dtos: list[competency_ranking_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[competency_ranking_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: competency_ranking_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[competency_ranking_rich_dto], dtos2: list[competency_ranking_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> competency_ranking_rich_dto | None:
        found_dtos = list(filter(lambda x: x.competency_ranking_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, competency_ranking_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.competency_ranking_uid] = dto
        return res
    def get_active(self) -> competency_ranking_rich_dtos:
        return competency_ranking_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> competency_ranking_rich_dtos:
        return competency_ranking_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> competency_ranking_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> competency_ranking_rich_dtos:
        if len(self.dtos) > n:
            return competency_ranking_rich_dtos(self.dtos[:n])
        else:
            return competency_ranking_rich_dtos(self.dtos)
    def get_last(self) -> competency_ranking_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, competency_ranking_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[competency_ranking_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[competency_ranking_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[competency_ranking_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[competency_ranking_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[competency_ranking_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[competency_ranking_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, competency_ranking_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, competency_ranking_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, competency_ranking_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[competency_ranking_rich_dto], bool]) -> competency_ranking_rich_dtos:
        return competency_ranking_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[competency_ranking_rich_dto], str]) -> dict[str, competency_ranking_rich_dtos]:
        grp_data: dict[str, competency_ranking_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = competency_ranking_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[competency_ranking_rich_dto], str], agg_method: Callable[[competency_ranking_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, competency_ranking_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[competency_ranking_rich_dto], bool]) -> competency_ranking_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[competency_ranking_rich_dto], any], compare_method: Callable[[any, any], bool]) -> competency_ranking_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class currency_rate_rich_dtos(base_dtos):
    dtos: list[currency_rate_rich_dto]
    def __init__(self, dtos: list[currency_rate_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[currency_rate_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: currency_rate_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[currency_rate_rich_dto], dtos2: list[currency_rate_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> currency_rate_rich_dto | None:
        found_dtos = list(filter(lambda x: x.currency_rate_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, currency_rate_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.currency_rate_uid] = dto
        return res
    def get_active(self) -> currency_rate_rich_dtos:
        return currency_rate_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> currency_rate_rich_dtos:
        return currency_rate_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> currency_rate_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> currency_rate_rich_dtos:
        if len(self.dtos) > n:
            return currency_rate_rich_dtos(self.dtos[:n])
        else:
            return currency_rate_rich_dtos(self.dtos)
    def get_last(self) -> currency_rate_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, currency_rate_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[currency_rate_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[currency_rate_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[currency_rate_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[currency_rate_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[currency_rate_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[currency_rate_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, currency_rate_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, currency_rate_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, currency_rate_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[currency_rate_rich_dto], bool]) -> currency_rate_rich_dtos:
        return currency_rate_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[currency_rate_rich_dto], str]) -> dict[str, currency_rate_rich_dtos]:
        grp_data: dict[str, currency_rate_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = currency_rate_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[currency_rate_rich_dto], str], agg_method: Callable[[currency_rate_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, currency_rate_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[currency_rate_rich_dto], bool]) -> currency_rate_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[currency_rate_rich_dto], any], compare_method: Callable[[any, any], bool]) -> currency_rate_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class currency_source_rich_dtos(base_dtos):
    dtos: list[currency_source_rich_dto]
    def __init__(self, dtos: list[currency_source_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[currency_source_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: currency_source_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[currency_source_rich_dto], dtos2: list[currency_source_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> currency_source_rich_dto | None:
        found_dtos = list(filter(lambda x: x.currency_source_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, currency_source_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.currency_source_uid] = dto
        return res
    def get_active(self) -> currency_source_rich_dtos:
        return currency_source_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> currency_source_rich_dtos:
        return currency_source_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> currency_source_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> currency_source_rich_dtos:
        if len(self.dtos) > n:
            return currency_source_rich_dtos(self.dtos[:n])
        else:
            return currency_source_rich_dtos(self.dtos)
    def get_last(self) -> currency_source_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, currency_source_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[currency_source_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[currency_source_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[currency_source_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[currency_source_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[currency_source_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[currency_source_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, currency_source_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, currency_source_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, currency_source_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[currency_source_rich_dto], bool]) -> currency_source_rich_dtos:
        return currency_source_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[currency_source_rich_dto], str]) -> dict[str, currency_source_rich_dtos]:
        grp_data: dict[str, currency_source_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = currency_source_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[currency_source_rich_dto], str], agg_method: Callable[[currency_source_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, currency_source_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[currency_source_rich_dto], bool]) -> currency_source_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[currency_source_rich_dto], any], compare_method: Callable[[any, any], bool]) -> currency_source_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class location_hierarchy_rich_dtos(base_dtos):
    dtos: list[location_hierarchy_rich_dto]
    def __init__(self, dtos: list[location_hierarchy_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[location_hierarchy_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: location_hierarchy_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[location_hierarchy_rich_dto], dtos2: list[location_hierarchy_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> location_hierarchy_rich_dto | None:
        found_dtos = list(filter(lambda x: x.location_hierarchy_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, location_hierarchy_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.location_hierarchy_uid] = dto
        return res
    def get_active(self) -> location_hierarchy_rich_dtos:
        return location_hierarchy_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> location_hierarchy_rich_dtos:
        return location_hierarchy_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> location_hierarchy_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> location_hierarchy_rich_dtos:
        if len(self.dtos) > n:
            return location_hierarchy_rich_dtos(self.dtos[:n])
        else:
            return location_hierarchy_rich_dtos(self.dtos)
    def get_last(self) -> location_hierarchy_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, location_hierarchy_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[location_hierarchy_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[location_hierarchy_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[location_hierarchy_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[location_hierarchy_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[location_hierarchy_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[location_hierarchy_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, location_hierarchy_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, location_hierarchy_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, location_hierarchy_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[location_hierarchy_rich_dto], bool]) -> location_hierarchy_rich_dtos:
        return location_hierarchy_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[location_hierarchy_rich_dto], str]) -> dict[str, location_hierarchy_rich_dtos]:
        grp_data: dict[str, location_hierarchy_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = location_hierarchy_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[location_hierarchy_rich_dto], str], agg_method: Callable[[location_hierarchy_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, location_hierarchy_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[location_hierarchy_rich_dto], bool]) -> location_hierarchy_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[location_hierarchy_rich_dto], any], compare_method: Callable[[any, any], bool]) -> location_hierarchy_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class location_region_rich_dtos(base_dtos):
    dtos: list[location_region_rich_dto]
    def __init__(self, dtos: list[location_region_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[location_region_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: location_region_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[location_region_rich_dto], dtos2: list[location_region_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> location_region_rich_dto | None:
        found_dtos = list(filter(lambda x: x.location_region_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, location_region_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.location_region_uid] = dto
        return res
    def get_active(self) -> location_region_rich_dtos:
        return location_region_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> location_region_rich_dtos:
        return location_region_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> location_region_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> location_region_rich_dtos:
        if len(self.dtos) > n:
            return location_region_rich_dtos(self.dtos[:n])
        else:
            return location_region_rich_dtos(self.dtos)
    def get_last(self) -> location_region_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, location_region_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[location_region_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[location_region_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[location_region_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[location_region_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[location_region_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[location_region_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, location_region_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, location_region_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, location_region_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[location_region_rich_dto], bool]) -> location_region_rich_dtos:
        return location_region_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[location_region_rich_dto], str]) -> dict[str, location_region_rich_dtos]:
        grp_data: dict[str, location_region_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = location_region_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[location_region_rich_dto], str], agg_method: Callable[[location_region_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, location_region_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[location_region_rich_dto], bool]) -> location_region_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[location_region_rich_dto], any], compare_method: Callable[[any, any], bool]) -> location_region_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class process_result_rich_dtos(base_dtos):
    dtos: list[process_result_rich_dto]
    def __init__(self, dtos: list[process_result_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[process_result_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: process_result_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[process_result_rich_dto], dtos2: list[process_result_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> process_result_rich_dto | None:
        found_dtos = list(filter(lambda x: x.process_result_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, process_result_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.process_result_uid] = dto
        return res
    def get_active(self) -> process_result_rich_dtos:
        return process_result_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> process_result_rich_dtos:
        return process_result_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> process_result_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> process_result_rich_dtos:
        if len(self.dtos) > n:
            return process_result_rich_dtos(self.dtos[:n])
        else:
            return process_result_rich_dtos(self.dtos)
    def get_last(self) -> process_result_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, process_result_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[process_result_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[process_result_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[process_result_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[process_result_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[process_result_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[process_result_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, process_result_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, process_result_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, process_result_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[process_result_rich_dto], bool]) -> process_result_rich_dtos:
        return process_result_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[process_result_rich_dto], str]) -> dict[str, process_result_rich_dtos]:
        grp_data: dict[str, process_result_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = process_result_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[process_result_rich_dto], str], agg_method: Callable[[process_result_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, process_result_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[process_result_rich_dto], bool]) -> process_result_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[process_result_rich_dto], any], compare_method: Callable[[any, any], bool]) -> process_result_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
...
@dataclass(frozen=False)
class project_phase_rich_dtos(base_dtos):
    dtos: list[project_phase_rich_dto]
    def __init__(self, dtos: list[project_phase_rich_dto]):
        super().__init__(dtos)
        self.dtos = dtos
    @classmethod
    def empty_list(cls):
        return cls(list())
    @classmethod
    def from_list(cls, dtos: list[project_phase_rich_dto]):
        return cls(dtos)
    @classmethod
    def from_object(cls, dto: project_phase_rich_dto):
        return cls([dto])
    @classmethod
    def from_lists(cls, dtos1: list[project_phase_rich_dto], dtos2: list[project_phase_rich_dto]):
        return cls(dtos1 + dtos2)
    def find_by_uid(self, uid: str) -> project_phase_rich_dto | None:
        found_dtos = list(filter(lambda x: x.project_phase_uid == uid, self.dtos))
        if (len(found_dtos)>0):
            return found_dtos[0]
        else:
            return None
    def map_by_uid(self) -> dict[str, project_phase_rich_dto]:
        res = {}
        for dto in self.dtos:
            res[dto.project_phase_uid] = dto
        return res
    def get_active(self) -> project_phase_rich_dtos:
        return project_phase_rich_dtos(list(filter(lambda x: x.is_active == 1, self.dtos)))
    def get_inactive(self) -> project_phase_rich_dtos:
        return project_phase_rich_dtos(list(filter(lambda x: x.is_active != 1, self.dtos)))
    def get_first(self) -> project_phase_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[0]
        else:
            return None
    def get_first_n(self, n: int) -> project_phase_rich_dtos:
        if len(self.dtos) > n:
            return project_phase_rich_dtos(self.dtos[:n])
        else:
            return project_phase_rich_dtos(self.dtos)
    def get_last(self) -> project_phase_rich_dto | None:
        if len(self.dtos) > 0:
            return self.dtos[-1]
        else:
            return None
    def to_dict_by_uid(self) -> dict[str, project_phase_rich_dto]:
        d = {}
        for dto in self.dtos:
            d[dto.get_uid()] = dto
        return d
    ...
    def for_first(self, do_method: Callable):
        if len(self.dtos) > 0:
            return do_method(self.dtos[0])
    def for_filter(self, check_method: Callable[[project_phase_rich_dto], bool], do_method: Callable):
        for dto in self.dtos:
            if check_method(dto):
                do_method(dto)
    def check_all(self, check_method: Callable[[project_phase_rich_dto], bool]) -> bool:
        init = True
        for dto in self.dtos:
            init = init and check_method(dto)
        return False
    def check_any(self, check_method: Callable[[project_phase_rich_dto], bool]) -> bool:
        init = False
        for dto in self.dtos:
            init = init or check_method(dto)
        return False
    def map_to_string(self, map_method: Callable[[project_phase_rich_dto], str]) -> list[str]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_int(self, map_method: Callable[[project_phase_rich_dto], int]) -> list[int]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def map_to_float(self, map_method: Callable[[project_phase_rich_dto], float]) -> list[float]:
        return list(map(lambda dto:  map_method(dto), self.dtos))
    def aggregate_string(self, map_method: Callable[[str, project_phase_rich_dto], str], init: str = "") -> str:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_int(self, map_method: Callable[[int, project_phase_rich_dto], int], init: int = 0) -> int:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def aggregate_float(self, map_method: Callable[[float, project_phase_rich_dto], float], init: float = 0) -> float:
        init_value = init
        for dto in self.dtos:
            init_value = map_method(init_value, dto)
        return init_value
    def filter(self, filter_method: Callable[[project_phase_rich_dto], bool]) -> project_phase_rich_dtos:
        return project_phase_rich_dtos(list(filter(filter_method, self.dtos)))
    def group_by(self, group_method: Callable[[project_phase_rich_dto], str]) -> dict[str, project_phase_rich_dtos]:
        grp_data: dict[str, project_phase_rich_dtos] = {}
        for dto in self.dtos:
            key = group_method(dto)
            grp_list = grp_data[key]
            if grp_list is None:
                grp_list = project_phase_rich_dtos([])
                grp_data[key] = grp_list
            grp_list.dtos.append(dto)
        return grp_data
    def aggregate_by(self, group_method: Callable[[project_phase_rich_dto], str], agg_method: Callable[[project_phase_rich_dtos], any]) -> dict[str, any]:
        grp_data: dict[str, project_phase_rich_dtos] = self.group_by(group_method)
        res: dict[str, any] = {}
        for key in grp_data:
            value = agg_method(grp_data[key])
            res[key] = value
        return res
    def find(self, check_method: Callable[[project_phase_rich_dto], bool]) -> project_phase_rich_dto | None:
        for dto in self.dtos:
            if check_method(dto):
                return dto
        return None
    def compare_by(self, value_method: Callable[[project_phase_rich_dto], any], compare_method: Callable[[any, any], bool]) -> project_phase_rich_dto | None:
        if len(self.dtos) == 0:
            return None
        else:
            max_value = value_method(self.dtos[0])
